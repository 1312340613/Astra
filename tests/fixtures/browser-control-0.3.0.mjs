...
const OPERATIONS = new Set(['tabs','attach','open','snapshot','click','type','fill','read','select','wait','screenshot','handoff','resume','close']);
...
export function createControl(api, {openTimeoutMs=10000}={}) {
  ...
  async function pageNow(id, operation, args, token) {
    ...
    const write=['click','type','fill','select'].includes(operation);
  ...
  return {
    ...
    async handle(request) {
      ...
      try {
        ...
        if (!OPERATIONS.has(operation) || !args || Array.isArray(args) || typeof args!=='object') throw new Error('Unsupported operation or arguments');
