...
Choose only `bindable=true` catalog windows. `ax_window_unmatched` means the window is present but has no unique AX match; it does not mean refs expired or the system prohibits interaction. Stop repeated catalog/bind calls while it is unchanged. After the panel state changes, refresh the catalog and obtain new refs; never reuse or implicitly rebind an old ref. Do not target an obscured parent as a substitute. A new ref remains bound to its own exact current catalog target.

File panels with the `AXSheet` role may expose their name as AXDescription when AXTitle is absent. Catalog, binding and foreground focus checks share that bounded fallback; other windows, failed reads and ambiguous matches do not gain that fallback. `unmatched_window_observed` in an action recovery result describes a possible blocker, not a new input target or permission to repeat the action.

Nested file panels such as Go to Folder require a bounded, directly owned sheet chain from the focused window. An unnamed sheet additionally needs a sole unnamed system window with matching PID, bounds and retained identity. Within sheets, buttons and children on the proven focus ancestry are read before other directory branches, preserving Open/Cancel and the current file column within the observation budget. Plain text events clear inherited shortcut modifiers; verify the resulting field value and selected filename, since dispatch acknowledgement alone does not prove either result.

Sheet screenshots are cropped from a verified capture of their directly owned root window. Only the selected sheet's pixels are published, with its own coordinate space; a changed or ambiguous parent fails the observation. This avoids system captures that shrink an entire parent window into the panel's requested image size. Local Edge acceptance covers file selection using both element refs and screenshot coordinates.
