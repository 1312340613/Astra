...
Run `./checkmail.sh <command> --json` on POSIX and `checkmail.bat <command> --json` on Windows. Use `sync`, `recent`, `search`, `read`, `folders`, and `status` according to the user's request. Explicit attachment download is POSIX-only: run `attachment --folder FOLDER --uidvalidity N --uid N --part PART` only for one user-selected attachment; on Windows report that attachment download fails closed and do not attempt another path.
